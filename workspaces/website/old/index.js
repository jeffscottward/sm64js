...
import { n64GfxProcessorInstance as GFX } from "../rom/src/graphics/n64GfxProcessor";
import { set_vp } from "../rom/src/include/config";
import * as Gbi from "../rom/src/include/gbi";
import { loadDataIntoGame } from "../rom/src/romTextureLoader.js";

import { GameInstance as Game } from "../rom/src/game/Game";
import { PrintInstance } from "../rom/src/game/Print.js";
import { viewport } from "../rom/src/game/Area";
import * as Mario from "../rom/src/game/Mario.js";
import * as cam from "../rom/src/game/Camera.js";

import { selfAdmin } from "../rom/src/mmo/chat";
import * as Socket from "../rom/src/mmo/socket.js";
import "../rom/src/mmo/cosmetics";
import "../rom/src/mmo/cmts_cosmetics";
...
const setStatsUpdate = setInterval(() => {
  ...
  if (!isNaN(window.sm64js.fps))
    fpsMeter.textContent = `FPS: ${window.sm64js.fps}`;
...
if (
  localStorage.getItem("widescreenOn") === 0 ||
  !localStorage.getItem("widescreenOn")
) {
  widescreenOff();
} else {
  widescreenOn();
...
window.onload = async () => {
  ...
  if (url_params.has("code")) {
    ...
    const res = await fetch(
      `${process.env.BACKEND_URL ?? ""}/api/login/${state.type}`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
